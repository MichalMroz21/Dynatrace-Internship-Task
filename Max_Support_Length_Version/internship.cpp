        ...
        */
        ...
        for(const auto& [id, product] : productData.items()) {
            ...
            for(const auto& [id, versionInfo] : product[VERSIONS_NAME].items()){
                ...
                int prevMaxSupportLength{maxSupportLength};

                maxSupportLength = std::max(maxSupportLength, supportLength); 
                atLeastOneVersionValid = true; 

                if(prevMaxSupportLength != maxSupportLength){ //if new max is found with std::max then update corresponding information for this version.
            ...
            if(atLeastOneVersionValid){ //if every version is invalid is some way (continue triggered), then don't add length = 0 to results.
                maxSupportLengths.push_back(make_tuple(maxSupportLength, cycleOfMaxReleaseVersion, productName));
